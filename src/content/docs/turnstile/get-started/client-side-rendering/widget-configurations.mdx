...
<Tabs>
	...
	<TabItem label="Explicit rendering">
		Explicit rendering gives you programmatic control over when and how widgets are created using JavaScript functions. It is best used for dynamic websites and single-page applications (SPAs), when you need to control timing of widget creation, conditional rendering based on user interactions, or for multiple widgets with different configurations.
...
- **`interaction-only`**: The widget becomes visible only when visitor interaction is required and provides the cleanest user experience. Most users will never see the widget, but suspected bots will encounter the interactive challenge.
...
:::note[Notes]
- When set to `auto`, Turnstile automatically detects the user's preferred language from their browser settings.
- If a requested language is not supported, Turnstile falls back to English.
- Language affects all user-facing text including loading messages, error states, and accessibility labels.
- Setting specific languages can improve user experience for international audiences.
:::
...
- Use error callbacks for graceful error handling and user feedback.
- Monitor expired tokens to refresh challenges before they become invalid.
- Handle timeouts to guide users through challenge resolution.
...
- **`auto`** (default): Automatically retries failed challenges. Auto retry provides better user experience by automatically recovering from temporary network issues or processing errors.
...
- `auto` refresh provides seamless user experience but uses more resources.
- `manual` refresh gives users control but requires them to take action.
- `never` refresh requires your application to handle all refresh logic.

Different strategies can be used for token expiration versus interactive timeouts based on your user experience requirements.
...
- **Automatic form integration**: This means that the token is included when the form is submitted, requiring no additional JavaScript. 
- **Custom field names**: This helps avoid conflicts with existing form fields.
- **Disabled response fields**: This gives you full control over token handling for complex form scenarios.
